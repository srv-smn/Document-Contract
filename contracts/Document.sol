...
contract Documents is Authorizable, ReentrancyGuard {
    ...
    event Registered(uint256 CertificateIndex, string contentURI);
    ...
    function registerDocument(string[] calldata _hash)
        public
        onlyAuthorized
        nonReentrant
    {
        for (uint256 i = 0; i < _hash.length; i++) {
            uint256 index = s_certificateIndex.current();
    ...
    function getDocumentDetails(uint256[] calldata _certificateID)
        public
        view
        returns (string[] memory)
    {
        string[] memory m_hash = new string[](_certificateID.length);
        uint256 totalDoc = totalCertificateRegistered();
        string memory m_baseURL = getBaseURL();
        for (uint256 i = 0; i < _certificateID.length; i++) {
            require(_certificateID[i] < totalDoc, "Certificate Id Invalid");
            //m_hash[i] = s_document[_certificateID[i]];
            m_hash[i] = bytes(m_baseURL).length > 0
                ? string(
                    abi.encodePacked(m_baseURL, s_document[_certificateID[i]])
                )
                : "";
        }
        return m_hash;
    ...
    function totalCertificateRegistered() public view returns (uint256) {
